...
for(let i = 0; i < data.length-1; i++){// data[i]: 0 -> timestring, 1 -> descrição da etapa.
    ...
    if(index >= 0){
        tempos[index].push((fimEtapa-inicioEtapa)/1000)
    }else{
        if(etapa == 'fim'){
            continue
        }
        etapas.push(etapa)
        tempos.push([(fimEtapa-inicioEtapa)/1000])
    }
}
...
tObs /= 1000// Tempo observado em s.
...
txtTempoObs.innerHTML = 'Tempo observado:' + ` ${tObs.toFixed(1)}` + ' s'
function evalTimes(){
    var tNormal = tObs*Number(ritmo.value)/100
    var tPadrao = tNormal*(1+Number(tol.value)/100)
    txtTempoNormal.innerHTML = 'Tempo normal:' + ` ${tNormal.toFixed(1)}` + ' s'
    txtTempoPadrao.innerHTML = 'Tempo padrão:' + ` ${tPadrao.toFixed(1)}` + ' s'
}
